...
时间谱默认使用 `fTime`。在 `0-800 ns` 内找最高峰，先从半高点估计粗略 FWHM，用该宽度做纯高斯 seed 拟合；最终联合拟合范围采用约 `2*FWHM`。随后用 `Gaussian + radioactive build-up + linear background` 联合拟合。build-up 项起点可移动，但限制在高斯峰心 `±15 ns` 内；起点之后按近似常数生成率同时衰变：`A*(1-exp(-(t-t0)/tau))`；直线本底的初始水平由 `200-250 ns` 旁带平均计数给出。输出中同时给出 `time_offset_to_442_ns = 442 - peak` 和 `time_gain_to_442 = 442 / peak`。
The default time branch is `fTime`. The strongest peak below `800 ns` is first given a rough FWHM estimate from half-height crossings, and that width defines the Gaussian seed fit. The final combined fit range is about `2*FWHM`. It is then refitted with `Gaussian + radioactive build-up + linear background`. The build-up start time is allowed to move, but is constrained within `±15 ns` of the Gaussian mean; after that start it follows constant production with decay: `A*(1-exp(-(t-t0)/tau))`. The linear-background level is seeded from the average bin content in `200-250 ns`. The output includes both `time_offset_to_442_ns = 442 - peak` and `time_gain_to_442 = 442 / peak`.
