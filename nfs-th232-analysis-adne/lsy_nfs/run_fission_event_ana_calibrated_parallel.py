#!/usr/bin/env python3
"""Parallel launcher for calibrated NFS fission analysis.

并行执行逐 run、逐 crystal 刻度后的 NFS fission 分析。
...
def discover_mult3_files(sources: list[str]) -> list[Path]:
    ...
    for source in sources:
        ...
        if p.is_dir():
            ...
            if p.name == "mult3_nfs_run_*.root" or (p.name.startswith("mult3_nfs_run_") and p.suffix == ".root"):
                files.append(p)
            else:
                # Keep explicit files for compatibility; ROOT will report if it is unusable.
                files.append(p)
        else:
...
def build_root_call(
    ...
    return (
        ...
        f'{distance:.12g},{time_fwhm:.12g},'
        f'"{root_escape(energy_bins)}",'
...
def run_job(job_id: int, cmd: list[str], log_path: Path) -> int:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run calibrated fission_event_ana on mult3 ROOT files in parallel / 并行运行 calibrated fission 分析",
    ...
    parser.add_argument("--distance", type=float, default=8.6, help="NFS distance in meters")
    parser.add_argument("--time-fwhm", type=float, default=20.0, help="time FWHM in ns")
    parser.add_argument("--energy-bins", default="2,4,7,12,20,30,50", help="neutron energy bin edges in MeV")
    parser.add_argument("--min-mult", type=int, default=2, help="minimum clover multiplicity after cuts")
    ...
    parser.add_argument("--macro", default=str(here / "fission_event_ana_calibrated.C"), help="ROOT macro path")
    ...
    for idx, chunk in enumerate(chunks, start=1):
        ...
        # Keep log path; close handle in parent after process exits through wait loop not required by Popen but okay.
        log_handle.close()
