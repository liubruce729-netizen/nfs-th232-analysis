...
//_____________________________________________________________________________
void CError::Infos(const char *message, const char *message2,
		const char* comment) {
...
//_____________________________________________________________________________
void CError::Infos(string *message, string *message2, string* comment) {
	// call :TreatError(int level,int status , char *message,char* comment )
	TreatError(0, 0, message, message2, comment);
}
//_____________________________________________________________________________
void CError::TreatError(int level, int status, string *message,
		string *message2, string* comment) {
	// call :TreatError(int level,int status , char *message,char* comment )
	if (message == NULL)
		return;
	if (message2 == NULL) {
		TreatError(level, status, (const char*) (message->data()));
		return;
	}
	if (comment == NULL) {
		TreatError(level, status, (const char*) message->data(),
				(const char*) message2->data());
		return;
	}
	TreatError(level, status, (const char*) message->data(),
			(const char*) message2->data(), comment->data());
}

//_____________________________________________________________________________

void CError::SetDebugVerbose(int level) {
	// Set Level of verbose  0<=level<=10
	if (level < 0 || level > 10)
		cout << "---> Warning , strange debug level :" << level << "\n";
	else
		fVerbose = level;
}
//_____________________________________________________________________________

void CError::TreatError(int level, int status, const char *message,
	...
	if ((level < 0 || level > 5))
		printf("--> Warning , strange error level : %d\n", level);
	/*
	 if ( level ==0) {
	 WorE = "--> Info   : ";
	 fNbInfo ++;
...
//_____________________________________________________________________________
void CError::TreatDebug(int level, int status, string *message,
		string *message2, string* comment) {
	// call :TreatDebug(int level,int status , string *message,string* comment )
	if (message == NULL)
		return;
	if (message2 == NULL) {
		TreatDebug(level, status, (const char*) (message->data()));
		return;
	}
	if (comment == NULL) {
		TreatDebug(level, status, (const char*) message->data(),
...
//_____________________________________________________________________________
void CError::TreatDebug(int level, int status, const char *message,
		const char *message2, const char* comment) {
	//         1 ->10
