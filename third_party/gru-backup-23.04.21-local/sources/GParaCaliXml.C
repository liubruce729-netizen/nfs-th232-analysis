...
GParaCaliXml::GParaCaliXml() {
	...
	void * test= gSystem->OpenDirectory(fFileName.Data());
	if (test==NULL)
...
void GParaCaliXml::InitXml(bool write_or_read, TString commentaire) {
	...
	if (write_or_read) {
		...
		if (fVerbose >3)fError.TreatError(0,0,tempos);
