...
{
   if ( OnSwap )
     AcqSwapInt32(&Buff->les_donnees.Count, 4);
...
{    
   ...
   AcqSwapInt16(Buff->les_donnees.cas.Buff, Size - IN2P3_HEADER_LEN);
...
{
   SwapNumBuf(Buff, Size);
   AcqSwapInt32(&Buff->les_donnees.cas.scale.Length, SCALE_HEADER_LEN);
   AcqSwapInt16(Buff->les_donnees.cas.scale.jbus_scale.Info_jbus, 
...
{
   SwapNumBuf(Buff, Size);
   AcqSwapInt32(&Buff->les_donnees.cas.scale.Length, 
...
static int SwTraiteFileh(in2p3_buffer_struct *Buff)
{
   ...
   extern bool AutoswBuf; /* Valeurs extraites en ligne de commande */
...
static int SwapEventdbSwapToHost(in2p3_buffer_struct *Buff, int Size)
{
  if (Machine_LSB) /* machine little-endian */
  {
    AcqSwapInt16((UNSINT16 *)Buff, Size);
  }
  else /* machine big-endian */
  {
    /* swappe Byte/Byte l'ident ASCII */
    AcqSwapInt16((UNSINT16 *)Buff,8);
...
static int SwapJbusToHost(in2p3_buffer_struct *Buff, int Size)
{
  if (Machine_LSB) /* machine little-endian */
  {
    ...
    AcqSwapInt32(&Buff->les_donnees.Count,24);
...
static int SwapScalerToHost(in2p3_buffer_struct *Buff, int Size)
{
  if (Machine_LSB) /* machine little-endian */
  {
    ...
    AcqSwapInt32(&Buff->les_donnees.Count,(Size - IN2P3_HEADER_LEN - 26));

    /* swappe Byte/Byte du checksum et du nombre d'evts */
    AcqSwapInt16( ( (UNSINT16 *)Buff + ((Size - 30) / 2) ) , 6 );
  }
...
static int SwapStatusSwapToHost(in2p3_buffer_struct *Buff, int Size)
{
  ...
  else /* machine big-endian */
  {
    ...
    AcqSwapInt16((UNSINT16 *)Buff->les_donnees.cas.status.VmeName, 16);
  }
...
static int SwapEbyedat(in2p3_buffer_struct *Buff, int Size)
{
  ...
  AcqSwapInt32((UNSINT32 *)&bufhd->num, 8);

  /* swappe source_id, dest_id, le numero de flux et le nombre d'evts */
  AcqSwapInt16((UNSINT16 *)&bufhd->source_id, 8);

  /* swappe le checksum 32 bits et la longueur utile du buffer */
  AcqSwapInt32((UNSINT32 *)&bufhd->checksum, 8);

  /* Swappe le reste du buffer */
  AcqSwapInt16((UNSINT16 *)((char *)Buff + GANIL_BUF_HD_SIZE), Size - GANIL_BUF_HD_SIZE );
