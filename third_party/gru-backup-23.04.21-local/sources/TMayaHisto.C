 ...
 */// fill by (x,y)
{
	...
	fHoneyCombBins->SetOwner();
TString tfname;
tfname.Form("_dfltRangeFunction_%s",name);

	fRangeFunction = new TF1(tfname,_dfltRangeFunction_, 0,
...
void TMayaHisto::CreateHoneyCombStructure() {
	...
	for (int i = 0; i < this->GetXaxis()->GetNbins(); i++) {
		for (int j = 0; j < this->GetYaxis()->GetNbins(); j++) {
			name_pad = TString("pad_") + (Long_t)(i + 1) + TString("_") + (Long_t)(
					j + 1) +TString("_") + name_histo;;
			bin = GetBin(i, j);
			x[0] = -0.5;
			x[1] = 0.0;
			x[2] = 0.5;
			...
			new ((*fHoneyCombBins)[bin]) TCutG(name_pad.Data(), 7, x, y);
...
//______________________________________________________________________________
TH1 *TMayaHisto::DrawCopy2(Option_t *option) const {
	// Draw copy.
	...
	if (newth2== NULL){
		std::cout << "--> Warning : TMayaHisto::DrawCopy : Spectrum not cloned \n";
	}
...
Bool_t TMayaHisto::FitTrack(Option_t *o, Int_t Cmin, Int_t Cmax, // limits in Columns
	...
	if (opt.Contains("sech")) {
		...
		Double_t b=Yg-a*Xg; // ordonn�e a l origine Y = aX +b
