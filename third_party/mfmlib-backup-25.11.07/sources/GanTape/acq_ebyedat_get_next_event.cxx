 ...
 * ajout d'un break en fin de case
 ...
 - EvtNum sur 32 bits
 LL le mars 2017
 ajout matacq_2 format = format matacq avec transmission de 2 info supplémentaires
...
int acq_ebyedat_get_next_event_r(UNSINT16 * Buffer, UNSINT16 ** EvtAddr,
	...
	// nbsubevt    : pointer to return number of subevents
	// EvtFormat   : EvtFormat asked  in our case   = EVCT_VAR or -1 just to have pointer on event
        // NbSystem    : Number of system ( hystoricaly 0=Exogam,1=Vamos,2Tiara), -1 when no information.
				...
				case EVCT_VAR:
					...
					do { /* scan the sub events */
						...
						if (NextSubEvent > *NextEvent) {
							...
							/* B. Raine 12 april 07 */
							*NbSystem        = (SubEventHd->StartToken >> 10) & 0x3f;
...
int acq_ebyedat_get_next_event(UNSINT16 * Buffer, UNSINT16 ** EvtAddr,
		int * EvtNum, int EvtFormat, int* nbsubevt, long long** timestamps,int*NbSystem) {
	...
	return (acq_ebyedat_get_next_event_r(Buffer, EvtAddr, EvtNum, EvtFormat,
			&CurrentEvent, &NextEvent, nbsubevt, timestamps,NbSystem));
}
