...
#ifndef boolbool
typedef enum BOOL {false,true} boolbool;
#endif
 boolbool AutoswBuf,Swbufon;
...
void GanSwapInt32(UNSINT32 *Buf, int NbOctets)
{
  SW_MOT32 Temp, *Mot32;
  int i, NbMots;
...
void GanSwapInt16(UNSINT16 *Buf, int NbOctets)
{
  SW_MOT16 Temp, *Mot16;
  int i, NbMots;
...
{
   if ( OnSwap )
     GanSwapInt32(&Buff->les_donnees.Count, 4);
...
{    
   ...
   GanSwapInt16(Buff->les_donnees.cas.Buff, Size - IN2P3_HEADER_LEN);
...
{
   SwapNumBuf(Buff, Size);
   GanSwapInt32(&Buff->les_donnees.cas.scale.Length, SCALE_HEADER_LEN);
   GanSwapInt16(Buff->les_donnees.cas.scale.jbus_scale.Info_jbus, 
...
{
   SwapNumBuf(Buff, Size);
   GanSwapInt32(&Buff->les_donnees.cas.scale.Length, 
...
static int SwTraiteFileh(in2p3_buffer_struct *Buff)
{
   ...
   extern boolbool AutoswBuf; /* Valeurs extraites en ligne de commande */
...
int acq_swap_buf(in2p3_buffer_struct *Buff, int Size)
{
  ...
  extern boolbool AutoswBuf,Swbufon; /* Valeurs extraites en ligne de commande    */
...
int SwapBufToHost(char *Buff, int Size)
{
  ...
  for ( Hd = HdList; (Hd->Header[0] != '\0') && (strncmp(Buff+1,Hd->Header +1,7)); Hd++ ) ;
...
static int SwapEventdbSwapToHost(in2p3_buffer_struct *Buff, int Size)
{
  if (Machine_LSB) /* machine little-endian */
  {
    GanSwapInt16((UNSINT16 *)Buff, Size);
  }
  else /* machine big-endian */
  {
    /* swappe Byte/Byte l'ident ASCII */
    GanSwapInt16((UNSINT16 *)Buff,8);
...
static int SwapJbusToHost(in2p3_buffer_struct *Buff, int Size)
{
  if (Machine_LSB) /* machine little-endian */
  {
    ...
    GanSwapInt32(&Buff->les_donnees.Count,24);
...
static int SwapScalerToHost(in2p3_buffer_struct *Buff, int Size)
{
  if (Machine_LSB) /* machine little-endian */
  {
    ...
    GanSwapInt32(&Buff->les_donnees.Count,(Size - IN2P3_HEADER_LEN - 26));

    /* swappe Byte/Byte du checksum et du nombre d'evts */
    GanSwapInt16( ( (UNSINT16 *)Buff + ((Size - 30) / 2) ) , 6 );
  }
...
static int SwapStatusSwapToHost(in2p3_buffer_struct *Buff, int Size)
{
  ...
  else /* machine big-endian */
  {
    ...
    GanSwapInt16((UNSINT16 *)Buff->les_donnees.cas.status.VmeName, 16);
  }
...
static int SwapEbyedat(in2p3_buffer_struct *Buff, int Size)
{
  ...
  /* swappe le numero de buffer et le magic */
  GanSwapInt32((UNSINT32 *)&bufhd->num, 8);

  /* swappe source_id, dest_id, le numero de flux et le nombre d'evts */
  GanSwapInt16((UNSINT16 *)&bufhd->source_id, 8);

  /* swappe le checksum 32 bits et la longueur utile du buffer */
  GanSwapInt32((UNSINT32 *)&bufhd->checksum, 8);

  /* Swappe le reste du buffer */
  GanSwapInt16((UNSINT16 *)((char *)Buff + GANIL_BUF_HD_SIZE), Size - GANIL_BUF_HD_SIZE );
