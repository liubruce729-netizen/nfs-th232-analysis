...
//_______________________________________________________________________________
void MFMScalerDataFrame::SetEventNumberFromScalerFrameData() {
...
void MFMScalerDataFrame::SetTimeStampFromScalerFrameData() {
	// Compute  Time Stamp
	fTimeStamp = 0;
...
void MFMScalerDataFrame::FillScalerWithIn2p3Buffer(char* ptcharbuf,uint64_t timestamp){
	...
	uint32_t nbitem=0;
 	...
 	uint32_t EventCounter=0;
        in2p3_buffer_struct *buf= (in2p3_buffer_struct*)ptcharbuf;
 	nbitem = buf->les_donnees.cas.scale.Nb_channel;
 	EventCounter = buf->les_donnees.Count;
 	uint16_t fReserve[3];
 	SetAttributs();
	int oldframesize = GetFrameSize();
	int framesize = nbitem*sizeof(MFM_ScalerData_Item)+SCALER_DATA_HEADERSIZE ;
    ...
    SetNbItem((int)nbitem);
    for (i = 0; i < GetNbItems(); i++) {  
	Label = (uint32_t) buf ->les_donnees.cas.scale.jbus_scale.UnScale[i].Label;
	Status= (int32_t)  buf ->les_donnees.cas.scale.jbus_scale.UnScale[i].Status;
	Count = (uint32_t) buf ->les_donnees.cas.scale.jbus_scale.UnScale[i].Count;
	Frequency  = (uint32_t) buf ->les_donnees.cas.scale.jbus_scale.UnScale[i].Freq;
	Tics  = (uint32_t) buf ->les_donnees.cas.scale.jbus_scale.UnScale[i].Tics;
	for (int j = 0; j < 3; j++) fReserve[j] = buf->les_donnees.cas.scale.jbus_scale.UnScale[j].Reserve[i]; 
	SetValues(i, Label, Count, Frequency, Status,Tics,acqStatus);
	}
   SetEventNumber(EventCounter);
   SetTimeStamp(timestamp);
		...
		for (i = 0; i < GetNbItems(); i++) {
			...
			if ((count != 0xFFFF)) {
				...
				case 'd':// decimal display
					sprintf(tempo, "|%5d|%20ld|%10ld|%5d|%20ld|%5d", label, (unsigned long)count,
							(unsigned long)frequency, status, (unsigned long)tics, acqstatus);
					break;
